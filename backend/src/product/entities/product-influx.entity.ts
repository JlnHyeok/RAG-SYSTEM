...
const MEASUREMENT_NAME = 'cnc_product';
...
@ObjectType()
export class ProductInflux extends InfluxField implements IInfluxModel {
  // Tags
  // WorkshopId: string = '';
  // LineId: string = '';
  // OpCode: string = '';
  ...
  async find(
    ...
    return (await influxQueryApi.collectRows<ProductInflux>(query)).sort(
      (a, b) => {
  ...
  findOne(influxService: InfluxService, date: Date) {
    ...
    return influxQueryApi.collectRows<ProductInflux>(query);
  }
