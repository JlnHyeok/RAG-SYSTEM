...
class AnswerGenerator:
    ...
    def _create_reasoning_prompt(
        ...
        if question_type in ["ALARM_QUERY", "ABNORMAL_QUERY"]:
            ...
            instructions = """
            1. **현황 파악**: 이상감지 건수, 유형별 분포, 최근 발생 이력을 정리하세요.
            2. **원인 추론**: 이상 발생 패턴이 있다면 가능한 원인을 추론하세요.
            3. **조치 제안**: 이상 해소를 위한 구체적인 조치 방안을 제안하세요.
            """
            ...
            instructions = """
            1. **현재 상태**: 가동 여부, 현재 부하, 속도 등 핵심 수치를 보고하세요.
            2. **정상 범위 비교**: 측정값이 정상 범위인지 판단하세요.
            3. **이상 징후**: 이상 징후가 있다면 주의 사항을 알려주세요.
            """
            tone = "✅ **전문적/객관적 톤**: 수치와 사실 위주로 명확하게 전달"
            ...
            instructions = """
            1. **설비 목록**: 등록된 설비의 코드, 명칭, 현황을 표 형식으로 정리하세요.
            2. **상태 확인**: 각 설비의 상태나 위치 정보를 함께 안내하세요.
            3. **추가 정보**: 필요시 임계치 설정이나 관련 공구 정보도 언급하세요.
            """
            ...
            instructions = """
            1. **생산 현황**: 생산량, CT(사이클타임), 품질 결과 등을 정리하세요.
            2. **통계 분석**: 평균, 최대, 최소 등 통계 데이터가 있다면 함께 보고하세요.
            3. **개선 포인트**: 생산성 향상을 위한 인사이트가 있다면 제안하세요.
            """
            ...
            instructions = """
            1. **공구 현황**: 등록된 공구의 코드, 명칭, 수명 정보를 정리하세요.
            2. **교체 시기**: 수명 잔여량이나 교체 필요 여부를 안내하세요.
            3. **관리 팁**: 공구 수명 연장을 위한 관리 포인트를 제안하세요.
            """
        ...
        return f"""{persona}
                    ...
                    ✅ **근거 명시**: 답변의 근거가 되는 데이터나 문서를 구분하여 언급하세요. (DB 조회 결과 vs 문서 내용)
