...
```
증강의 핵심: 검색된 정보를 "어떻게" LLM에게 전달할지 설계

예시:
"이 문서의 이 부분을 강조해서 LLM에게 전달하자"
"출처와 신뢰도를 함께 표시해서 전달하자"
"텍스트와 이미지를 함께 구성해서 전달하자"
```
...
```
RAG에서의 생성:
- 기존 생성: 훈련 데이터 기반의 창의적 텍스트 생성
- RAG 생성: 검색된 사실 정보 기반의 정확한 답변 생성

차이점:
- 기존: "이런 내용으로 답변을 만들어볼까?" (창의성 중심)
- RAG: "이 사실들을 바탕으로 정확하게 답변하자" (정확성 중심)
```

**생성의 역할:**

```
생성의 핵심: 구성된 정보를 "어떻게" 자연스러운 답변으로 변환할지 실행

예시:
"전달받은 문서들을 바탕으로 사용자가 이해하기 쉬운 답변 작성"
"출처를 명확히 표시하면서 답변 구성"
"불충분한 정보는 솔직하게 표현"
```
...
```
입력: 텍스트, 이미지, 음성 등의 비정형 데이터
출력: 고차원 벡터 (예: 384차원, 768차원, 1536차원)
목적: 의미적으로 유사한 것들은 벡터 공간에서 가까이 위치하도록 변환
```
